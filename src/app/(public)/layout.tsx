        ...
        <div className='absolute right-0 top-full mt-2 w-56 glass-heavy rounded-[var(--radius-lg)] p-2 animate-slide-up shadow-lg shadow-black/40'>
          {links.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className='block px-4 py-3 rounded-[var(--radius-sm)] text-sm font-medium text-[var(--fg-secondary)] hover:text-[var(--fg-primary)] hover:bg-[var(--bg-tertiary)] transition-all'
            >
              {link.label}
            </Link>
          ))}
